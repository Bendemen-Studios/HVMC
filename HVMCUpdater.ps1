...
function Find-Java {
    ...
    if (Test-Path -LiteralPath $runtimeRoot) {
        $r = Get-ChildItem -LiteralPath $runtimeRoot -Filter 'java.exe' -File -Recurse -ErrorAction SilentlyContinue | Select-Object -First 1
    ...
    foreach ($pattern in @(
        ...
        $c = Get-ChildItem -Path $pattern -File -ErrorAction SilentlyContinue | Select-Object -First 1
...
function Ensure-Fabric {
    ...
    # Remove the old loader that caused the incompatibility dialog. The launcher must use 0.19.2.
    $oldFabricDir = Join-Path $versionsRoot "fabric-loader-0.18.1-$McVersion"
    if (Test-Path -LiteralPath $oldFabricDir) {
        Log "Oude Fabric 0.18.1 installatie verwijderen..."
        Remove-Item -LiteralPath $oldFabricDir -Recurse -Force -ErrorAction SilentlyContinue
    }
