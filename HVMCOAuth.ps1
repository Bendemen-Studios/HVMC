...
function Request-Token([string]$RefreshToken) {
    Invoke-RestMethod -Uri "$authority/oauth2/v2.0/token" -Method Post -ContentType 'application/x-www-form-urlencoded' -Body @{ client_id=$clientId; grant_type='refresh_token'; refresh_token=$RefreshToken; scope=$scope } -TimeoutSec 30
}
function Device-Login {
    $device = Invoke-RestMethod -Uri "$authority/oauth2/v2.0/devicecode" -Method Post -ContentType 'application/x-www-form-urlencoded' -Body @{ client_id=$clientId; scope=$scope } -TimeoutSec 30
    Add-Type -AssemblyName PresentationFramework
    [System.Windows.MessageBox]::Show("HVMC Microsoft-login`n`n$($device.message)", 'HVMC Login') | Out-Null
    $interval = [Math]::Max([int]$device.interval,5); $deadline=(Get-Date).AddSeconds([int]$device.expires_in)
    while ((Get-Date) -lt $deadline) {
        Start-Sleep -Seconds $interval
        try { return Invoke-RestMethod -Uri "$authority/oauth2/v2.0/token" -Method Post -ContentType 'application/x-www-form-urlencoded' -Body @{ client_id=$clientId; grant_type='urn:ietf:params:oauth:grant-type:device_code'; device_code=[string]$device.device_code } -TimeoutSec 30 }
        catch { $text=$_.ErrorDetails.Message; if($text -match 'authorization_pending'){continue}; if($text -match 'slow_down'){$interval+=5;continue}; throw }
    }
...
$profile=$null
try{$profile=Invoke-RestMethod -Uri 'https://graph.microsoft.com/v1.0/me?$select=id,displayName,userPrincipalName,mail' -Headers @{Authorization="Bearer $($token.access_token)"} -TimeoutSec 30}catch{$profile=Decode-JwtPayload ([string]$token.id_token)}
